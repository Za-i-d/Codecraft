...
restart_quiz.onclick = ()=>{
    ...
    widthValue = 0;
    userScore2 = 0;
    puntajeElem.textContent = userScore2;
...
function flashColor(element, color) {
    ...
    setTimeout(() => {
        element.style.transition = 'background-color 1.2s ease-out';
...
function optionSelected(answer){
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        ...
        flashColor(document.body, '#01ff3c');
        ...
        lives --;
        updateLives();
        flashColor(document.body, '#f94453');
    ...
    if (lives === 0) {
        setTimeout(() => {
            alert("Has perdido todas tus vidas. Serás redirigido a la vista de cursos.");
            window.location.href = "/cursos";  // redirigir a la vista de cursos
        }, 500);
    }
}

function updateLives() {
    if (lives < 3) document.getElementById("heart3").classList.add("lost-life");
    if (lives < 2) document.getElementById("heart2").classList.add("lost-life");
    if (lives < 1) document.getElementById("heart1").classList.add("lost-life");
}
